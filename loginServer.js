...
app.post('/login', (req, res) => {
    ...
    if (!userId || !token) {
        return res.status(400).json({ message: 'Votre ID et TOKEN sont obligatoires.' });
    }

    if (!isValidUserId(userId)) {
        return res.status(400).json({ message: "L'ID utilisateur est invalide. Seuls les nombres à 17-19 chiffres sont acceptés." });
    }

    if (!isValidTokenFormat(token)) {
        return res.status(400).json({ message: "Le format du token est invalide." });
    }
...
app.listen(PORT, () => {
    console.log(`Connexion activé : https://nexus-sb.online/`);
});
